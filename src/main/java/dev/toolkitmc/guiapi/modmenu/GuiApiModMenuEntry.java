...
public class GuiApiModMenuEntry implements ModMenuApi {
    ...
    static class GuiApiConfigScreen extends Screen {
        ...
        public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
            ...
            if (maxScrollY > 0) {
                ...
                int thumbHeight = Math.max(15, (int)((double)trackHeight / (trackHeight + maxScrollY) * trackHeight));
    ...
    static class GuiSaveProcessScreen extends Screen {
        ...
        public void renderBackground(DrawContext context, int mouseX, int mouseY, float delta) {
            // Safe No-Op: Bypasses vanilla background blur to avoid "Can only blur once per frame" crash
        }
        ...
        public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
            ...
            switch (state) {
                ...
                case SAVING_PROGRESS -> {
                    String status = ticksElapsed >= 20 ? "Overwriting Datapack JSON File..." : "Locating Datapack Folder...";
    ...
    static class ButtonListScreen extends Screen {
        ...
        private final net.minecraft.util.Identifier id;
        private final GuiDefinition def;
        final List<GuiDefinition.Button> buttonsList;
    ...
    static class ButtonEditorScreen extends Screen {
        ...
        ButtonEditorScreen(ButtonListScreen parent, int index, GuiDefinition.Button btn) {
            ...
            this.conditionText = btn.condition().isPresent()
                    ? btn.condition().get().type().name().toLowerCase() + ":" + btn.condition().get().value()
                    : "";
        }
        ...
        /** Slot çakışması varsa aynı page'deki ilk boş slotu döndürür. */
        private int resolveSlot(int targetSlot, int targetPage) {
            boolean duplicate = false;
            for (int i = 0; i < parent.buttonsList.size(); i++) {
                if (i == index) continue;
                GuiDefinition.Button existing = parent.buttonsList.get(i);
                if (existing.slot() == targetSlot && existing.page() == targetPage) {
                    duplicate = true;
                    break;
                }
            }
            if (!duplicate) return targetSlot;
            ...
            for (int s = 0; s < 54; s++) {
                ...
                for (int i = 0; i < parent.buttonsList.size(); i++) {
                    if (i == index) continue;
                    GuiDefinition.Button existing = parent.buttonsList.get(i);
                    if (existing.slot() == s && existing.page() == targetPage) {
                        slotUsed = true;
                        break;
                    }
                }
                if (!slotUsed) return s;
            }
            return targetSlot; // son çare: tüm slotlar dolu
        }
        ...
        protected void init() {
            ...
            addDrawableChild(ButtonWidget.builder(Text.literal("Apply"), b -> {
                ...
                int finalSlot = resolveSlot(slot, pageVal);
    ...
    static class ToggleEditorScreen extends Screen {
        ...
        private TextFieldWidget actionsOnField;
        private TextFieldWidget actionsOffField;
